...
{
    public class PartitionEqualSubSetSummemonization
    {
        public static bool canPartition(int[] num)
        {
            ...
            if (sum % 2 != 0)
                return false;
            bool[,] dp = new bool[num.Length, sum / 2 + 1];
            return canPartitionRecursive(dp,num, 6, 0);
        }

        private static bool canPartitionRecursive(bool[,] dp ,int[] num, int sum, int currentIndex)
        {
            // base check
            if (sum == 0)
                return true;
